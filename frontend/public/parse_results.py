...
def extract_exam_info(page) -> Dict[str, str]:
    ...
    # Extract Program 
    # Try to capture everything between "OFFICE REGISTER FOR THE" and "( Semester"
    # We use re.DOTALL to handle multiline if needed, though we joined lines above.
    ...
    if prog_match:
        ...
        # Fallback: Capture until first parenthesis if "( Semester" pattern fails
        prog_match = re.search(r'OFFICE REGISTER FOR THE\s+(.+?)\s*\(', clean_text, re.IGNORECASE)
        if prog_match:
            info['program'] = prog_match.group(1).strip()
